          ...
          for (var j = 0; j < nb; j++) {
            ...
            if (overR.editable && !overR.isDefault) {
              ...
              if (overR.property == "image") {
                ...
                var imgName = NSString.stringWithFormat("%@.png", overR.id);
                ...
                var currentOver = Xml.xmlAddElement(overrideParent, "override", ["property", overR.property, "value", overR.value, "on", overrideTargetPath], null);
                ...
                var _currentOver = Xml.xmlAddElement(overrideParent, "override", ["property", overR.property, "value", overR.value, "on", overrideTargetPath], null);
...
function isLine(layer) {
  return layer && layer.sketchObject && layer.sketchObject.isKindOfClass(MSShapePathLayer.class()) && layer.shapeType == "Custom" && layer.points.length == 2;
}
;
...
function isSupportedLayer(layer) {
  return isTextLayer(layer) || isBitmapLayer(layer) || (isRectangleShape(layer) || isOvalShape(layer) || isLine(layer)) && isSupportedShapeStyle(layer.style) || isLayerGroup(layer) || isArtboard(layer) || isSymbolInstance(layer);
}
...
Utils.exportLayerWithFormats = function (layer, exportPath, exportName, formats, preserveFrameSize, applyTransformAndOpacity) {
  ...
  if (formats.length > 0) {
    ...
    var isRotated = layer.transform.rotation != 0;
    ...
    if (!applyTransformAndOpacity && (layer.style.opacity != 1 || isRotated || isFlippedHorizontally || layer.transform.flippedVertically || parentIsGroup)) {
      useDuplicate = true;
    ...
    var absoluteInfluenceRect = nativeLayer.absoluteInfluenceRect();
