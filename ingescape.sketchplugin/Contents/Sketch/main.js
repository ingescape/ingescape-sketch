...
function onExportLibraryAsXml(context) {
  ...
  var documentPrefix = documentName;
  if (docPathElements.length > 0) {
    var rawDocumentName = docPathElements[docPathElements.length - 1].replace(/.sketch/g, "");
    documentName = Tree.getNameFromDocumentName(rawDocumentName);
    documentPrefix = Tree.pascalize(rawDocumentName);
  }

  // Init directories
  var libraryFontsSubDir = "resources/" + documentPrefix + "_library/fonts/";
  var libraryImagesSubDir = "resources/" + documentPrefix + "_library/images/";
  ...
  var documentContext = Tree.initDocumentContext(libraryImagesSubDir);
  ...
  //export model to XML
  var xmlFilePath = path + documentPrefix + "_library.xml";
...
function onExportApplicationAsXml(context) {
  ...
  // Init directories
  var appFontsSubDir = "resources/" + documentPrefix + "_app/fonts/";
  var appImagesSubDir = "resources/" + documentPrefix + "_app/images/";
  var libraryImagesSubDir = "resources/" + documentPrefix + "_library/images/";
  ...
  Tree.clearTempContext();
  var xmlFilePath = path + documentPrefix + "_app.xml";
...
function treeIterateLayers(layers, parent, shallIgnoreSymbolMasters, rootDirPath, imagesSubDir, xmlRoot, documentContext, mask) {
  ...
  for (var i = 0; layers && i < layers.length; i++) {
    ...
    if (actualType == "includeLibrary") {
      ...
      if (keyValues.length > 0) Xml.xmlAddCustomPropertiesToElement(xmlCurrent, keyValues);
    } else if (actualType == "includeExternal") {
      var _xmlCurrent = Xml.xmlAddElement(actualParent, "external", ["name", actualName], null);
      if (keyValues.length > 0) Xml.xmlAddCustomPropertiesToElement(_xmlCurrent, keyValues);
      ...
      if (keyValues.length > 0) Xml.xmlAddCustomPropertiesToElement(_xmlCurrent2, keyValues);
    } else {
      ...
      if (type == "MSSymbolInstance") {
            ...
            if (overR.property == "image" || overR.property == "textStyle" || overR.property == "stringValue" || overR.property == "layerStyle" || overR.property == "symbolID") {
              ...
              if (overR.property == "image") {
                ...
                if (TEMP_CONTEXT.imageHashToFilePath.hasOwnProperty(imgHash)) {
                  ...
                  var targetName = getUniqueNameFromIndexedData(indexedDataOverride);
        ...
        Xml.xmlAddAttributesToElement(_xmlCurrent3, ["symbolMaster", getUniqueNameFromIndexedData(indexedDataSymbolMaster), "type", actualType, "symbolMasterWidth", Math.ceil(l.master.frame.width), "symbolMasterHeight", Math.ceil(l.master.frame.height)]);
      ...
      if (keyValues.length > 0) Xml.xmlAddCustomPropertiesToElement(_xmlCurrent3, keyValues);
...
function treeAddAttributesFromLayerData(name, currentXMLElement, type, layer, rootDirPath, imagesSubDir, isCustomClass, currentMask, actualType) {
  ...
  */
  ...
  if (type == "MSTextLayer") {
    var text = layer.text;
    if (text) Xml.xmlAddElement(currentXMLElement, "content", null, escapeCharactersInUnicodePrivateUseArea(text));
...
function treeExportBitmapLayer(layer, currentXMLElement, rootDirPath, imagesSubDir, name, type) {
  if (isBitmapLayer(layer) && layer.image && isEmptyStyle(layer.style)) {
    ...
    var Xml = __webpack_require__(/*! ./xml.js */ "./src/xml.js");
    var imgHash = layer.image.sketchObject.hash();
    var imgRelativePath = "";
    if (TEMP_CONTEXT.imageHashToFilePath.hasOwnProperty(imgHash)) {
      imgRelativePath = TEMP_CONTEXT.imageHashToFilePath[imgHash];
    } else {
      ...
      var imgUid = camelize(name) + "__" + layer.sketchObject.objectID();
      imgRelativePath = imagesSubDir + imgUid + ".png";
      var imgData = layer.image.nsdata;
      imgData.writeToFile_atomically(NSString.stringWithFormat("%@%@", rootDirPath, imgRelativePath), false);
      TEMP_CONTEXT.imageHashToFilePath[imgHash] = imgRelativePath;
    }
    Xml.xmlAddAttributesToElement(currentXMLElement, ["hasExport", 1, "type", "image", "opacity", layer.style.opacity, "topInset", 0, "bottomInset", 0, "leftInset", 0, "rightInset", 0]);
    Xml.xmlAddElement(currentXMLElement, "export", ["format", "png", "size", "1x", "prefix", "", "suffix", "", "file", imgRelativePath], null);
  } else {
  ...
  if (layer.exportFormats.length > 0) {
    ...
    if (sketchVersion >= 97) {
      ...
      layer.exportFormats.forEach(function (format) {
        ...
        if (format.prefix) prefix = format.prefix;else if (format.suffix) suffix = format.suffix;
        var fileFormat = format.fileFormat;
        var fileName = prefix + name + suffix + "." + fileFormat;
...
function validSwatchName(rawName, colorNames) {
  ...
  var keyName = swatchNameKey(name);
  var validName = name;
  if (colorNames.has(keyName)) {
    var index = colorNames.get(keyName);
...
function escapeCharactersInUnicodePrivateUseArea(str) {
  var temp = removeIllegalXmlCharacters(str);
  temp = temp.replace(/\\/g, "\\\\");
  ...
  return temp.replace(/[\uE000-\uF8FF]/g, function (c) {
...
function isEmptyStyle(style) {
  var isEmpty = true;
  if (style) {
    isEmpty = style.blendingMode == "Normal" && style.fills.filter(function (item) {
      return item.enabled;
    }).length == 0 && style.borders.filter(function (item) {
      return item.enabled;
    }).length == 0 && style.shadows.filter(function (item) {
      return item.enabled;
    }).length == 0 && style.innerShadows.filter(function (item) {
      return item.enabled;
    }).length == 0 && !style.blur.enabled;
  }
  return isEmpty;
}
module.exports = {
  ...
  getNameFromDocumentName: getNameFromDocumentName,
...
Utils.showInFinder = function (directoryPathOrFilePath) {
  ...
  if (result) {
    var isDir = false;
    if (result.respondsToSelector("isDirectory")) isDir = result.isDirectory();else isDir = result.NSFileType === NSFileTypeDirectory;
    if (isDir) NSWorkspace.sharedWorkspace().openFile_withApplication(directoryPathOrFilePath, "Finder");else NSWorkspace.sharedWorkspace().selectFile_inFileViewerRootedAtPath(directoryPathOrFilePath, nil);
...
Utils.layerAbsoluteRect = function (layer) {
  ...
  if (sketchVersion >= 97) {
    ...
    var absoluteRect = layer.frame.changeBasis({
      from: layer.parent
    });
...
Utils.layerAbsoluteInfluenceRect = function (layer) {
  ...
  if (sketchVersion >= 97) {
    ...
    var nativeLayer = layer.sketchObject;
    var request = MSExportRequest.exportRequestsFromLayerAncestry_(nativeLayer.ancestry()).firstObject();
    request.setRect_(NSZeroRect);
    var exporter = MSExporter.exporterForRequest_colorSpace_(request, nil);
    exporter.trimmedBounds();
    return request.rect();
  } else if (sketchVersion >= 96) {
    // Sketch 96: method absoluteInfluenceRect() of MSLayer has been removed
    var _nativeLayer = layer.sketchObject;
    var document = _nativeLayer.documentData();
    var immutable = _nativeLayer.immutableModelObject();
    var relativeInfluenceRect = immutable.influenceRectForBoundsInDocument(document);
    return _nativeLayer.convertRect_toLayer_(relativeInfluenceRect, null);
  } else {
    // Sketch 95 and below
    var _nativeLayer2 = layer.sketchObject;
    return _nativeLayer2.absoluteInfluenceRect();
  }
};
  ...
  if (formats.length > 0) {
    ...
    if (!applyTransformAndOpacity && (layer.style.opacity != 1 || isRotated || isFlippedHorizontally || isFlippedVertically || parentIsGroup)) {
      ...
      if (isRotated) {
        ...
        } else layerToExport.transform.rotation = 0;
      }
    }
    var absoluteRect = Utils.layerAbsoluteRect(layerToExport);
    var absoluteInfluenceRect = Utils.layerAbsoluteInfluenceRect(layerToExport);
    ...
    if (!preserveFrameSize && !CGRectContainsRect(absoluteRect, absoluteInfluenceRect)) {
      ...
      var exportRectX = Utils.roundBy(exportRect.origin.x, 0.5);
      var exportRectY = Utils.roundBy(exportRect.origin.y, 0.5);
      var exportRectWidth = Math.ceil(exportRect.size.width);
      var exportRectHeight = Math.ceil(exportRect.size.height);
      ...
      var _exportRectX = Utils.roundBy(exportRect.origin.x, 0.5);
      var _exportRectY = Utils.roundBy(exportRect.origin.y, 0.5);
      var _exportRectWidth = Math.ceil(exportRect.size.width);
      var _exportRectHeight = Math.ceil(exportRect.size.height);
      exportRect = NSMakeRect(_exportRectX, _exportRectY, _exportRectWidth, _exportRectHeight);
    }
    ...
    if (sketchVersion >= 97) {
      ...
      formats.forEach(function (format) {
        ...
        var filePath = exportName;
        if (exportFormat.namingScheme() == 0) filePath += exportFormat.name();else filePath = exportFormat.name() + filePath;
